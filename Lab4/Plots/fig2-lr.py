...
# Import the data
frequency_kHz, inputPkPk, outputPkPk, phase_us = np.genfromtxt("Lab4/Data/lr.csv",delimiter=',', skiprows=2, usecols=(0,1,2,3), unpack=True)
...
Av_theory = 1 / np.sqrt(1 + (frequency_angular * inductor / resistor)**2)

# Find the phase difference
Phi = -2 * 180 * phase_s * frequency
Phi_theoretical = -1 * (np.arctan(frequency_angular * inductor / resistor)) * 180 / np.pi
