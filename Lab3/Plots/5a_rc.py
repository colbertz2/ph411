...
frequency, input_v, output_v, phase_time = np.genfromtxt('../Data/Part5_rcCircuit.csv', delimiter=',', usecols=(0,1,2,3), skiprows=2, unpack=True) # FREQUENCY IN KHZ
...
theory_Av = 1 / np.sqrt((2 * np.pi * theory_frequency * R * C)**2 + 1)

# Theoretical phase difference
theory_phase = (-1 * np.arctan(2 * np.pi * theory_frequency * R * C) * 180 / np.pi)

# Amplitude plot
plt.subplot(211)
plt.title("RC Circuit with Sine Wave Input \nAmplitude")
